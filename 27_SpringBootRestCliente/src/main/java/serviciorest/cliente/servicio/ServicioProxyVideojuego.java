...
public class ServicioProxyVideojuego {
	...
	/**
	 * Metodo que obtiene una persona del servicio REST a partir de un id En caso de
	 * que el id no exita arrojaria una expcepcion que se captura para sacar el
	 * codigo de respuesta
	 * 
	 * @param id que queremos obtener
	 * @return retorna la persona que estamos buscando, null en caso de que la
	 *         persona no se encuentre en el servidor (devuelva 404) o haya habido
	 *         algun otro error.
	 */
	 ...
	 * 
	 * @param p la persona que queremos modificar, se hara a partir del id por lo
	 *          que tiene que estar relleno.
	 * @return true en caso de que se haya podido modificar la persona. false en
	 *         caso contrario.
	 */
	 ...
	 * @return true en caso de que se haya podido borrar la persona. false en caso
	 *         contrario.
	 */
	...
	/**
	 * Metodo que devuelve todas las personas o todas las personas filtradas por
	 * nombre del web service
	 * 
	 * @param nombre en caso de ser distinto de null, devolvera el listado filtrado
	 *               por el nombre que le hayamos pasado en este parametro. En caso
	 *               de que sea null, el listado de las personas sera completo
	 * @return el listado de las personas segun el parametro de entrada o null en
	 *         caso de algun error con el servicio REST
	 */
	public List<Videojuego> listar(String nombre) {
		...
		try {
			...
			ResponseEntity<Videojuego[]> response = restTemplate.getForEntity(URL + queryParams, Videojuego[].class);
			Videojuego[] arrayJuegos = response.getBody();
			return Arrays.asList(arrayJuegos);// convertimos el array en un arraylist
		} catch (HttpClientErrorException e) {
